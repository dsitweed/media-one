...
public class Warehouse {
    ...
    public void addStaff(Staff newStaff) {
        ...
        if (listStaff.contains(newStaff) == false)
            listStaff.add(newStaff);
    }
//    Index in listStaff
    public void setStaff(Staff oldStaff, Staff newStaff) {
        Staff s;
        for (int i = 0; i < listStaff.size(); i++) {
            s = listStaff.get(i);
            if (s.equals(oldStaff)) listStaff.set(i, newStaff);
        }
    }
    ...
    public double allMoneyFormBill(int month, int year) {
        ...
        if (month == 0 && year == 0) {
            for (Bill bill : listBill) {
                //Tiền lời từ Hóa đơn bằng tổng tiền - tiền nhập
                total += bill.totalCost() - bill.totalImportMoney();
            }
            return total;
        }
        if (month == 0) {
...
//            System.out.println("Max value: " + buffer);
            ...
            j = startMonth;
            while (i <= endYear) {
                for ( ; j <= endMonth || i <= endYear; j++) {
                    System.out.printf("%40s|%40s|%50s\n", "NonFixedFee + MaintenanceFee",
                            allNonFixedFee(j, i) + allMaintenanceFee(j, i),
                            "Tháng " + j + " Năm" + i);
                    profit -= allNonFixedFee(j, i) + allMaintenanceFee(j, i);
                    if (j >= endMonth && i == endYear) break;
                    if (j == 12) break;
                }//end for month
                if (i < endYear) j = 1;
                i++;
            ...
            while (i <= endYear) {
                for ( ; j <= endMonth || i <= endYear; j++) {
                    System.out.printf("%40s|%40s|%50s\n", "Thu từ khách hàng",
                            allMoneyFormBill(j, i),
                            "Tháng " + j + "Năm" + i);
                    profit += allMoneyFormBill(j, i);
                    if (j >= endMonth && i == endYear) break;
                    if (j == 12) break;
                }//end for month
                if (i < endYear) j = 1;
